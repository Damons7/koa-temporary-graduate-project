...
module.exports = {
  ...
  GetPromoProductById: async categoryID => {
    return await Product.find({ category_id: categoryID, product_state: 1 }).limit(7);
  },
  //根据商品id查询商品数据
  GetProductByProductId: async id => {
    return await Product.find({ product_id: id, product_state: 1 });
  },
  //(订单查询)根据商品id查询商品数据
  GetProductByProductId_order: async id => {
    return await Product.find({ product_id: id });
  },
  //根据商品上架商家查询商品数据
  GetAddProductByFromUser: async from_user => {
    return await Product.find({ from_user: from_user, product_state: 1 });
  },
  // 连接数据库,根据商品id,获取商品图片
  ...
  GetProductByCategory: async (categoryID, offset = 0, rows = 0) => {
    return await Product.find({ category_id: categoryID[0], product_state: 1 })
  ...
  GetProductBySearch: async (search, offset = 0, rows = 0) => {
    ...
    return await Product.find({ product_state: 1, $or: [{ product_name: _search }, { product_title: _search }, { product_intro: _search }] })
      .sort({ product_selling_price: 1 }).skip(offset).limit(rows);
  },
  //上架商品
  ...
  //更新商品销量
  UpdateProductSales: async (product_id) => {
    return await Product.updateOne({ product_id: product_id }, { $set: { updateDate: new Date() }, $inc: { product_sales: +1 } })
  },
  //更新上架商品详情图片
